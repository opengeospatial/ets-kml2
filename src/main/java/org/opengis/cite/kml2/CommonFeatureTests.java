...
/**
 * Checks constraints that apply to any KML feature (an element that substitutes directly
 * or indirectly for kml:AbstractFeatureGroup).
 * <ul>
 * <li>ATC-106</li>
 * <li>ATC-108</li>
 * <li>ATC-111</li>
...
public class CommonFeatureTests extends CommonFixture {
	...
	/**
	 * <p>
	 * Constructor for CommonFeatureTests.
	 * </p>
	 */
	 ...
	 * constraints:
	 * <ol>
	 * <li>its value is a valid relative or absolute URL that refers to a shared style
	 * definition (any element that substitutes for kml:AbstractStyleSelectorGroup)</li>
	 * <li>if the reference is an absolute URI, it conforms to the 'http' or 'file' URI
	 * schemes</li>
	...
	public void validStyleReference() {
		...
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			NodeList styleUrlElem = kmlFeature.getElementsByTagNameNS(KML2.NS_NAME, "styleUrl");
			...
			if (styleUrl.getPath().isEmpty()) {
				...
				Assert.assertTrue(this.sharedStyles.contains(id),
						ErrorMessage.format(ErrorMessageKeys.SHARED_STYLE_NOT_FOUND, id));
				continue;
			}
			...
			try {
				...
				throw new AssertionError(ErrorMessage.format(ErrorMessageKeys.XML_ERROR, styleUrl));
			}
			Assert.assertNotNull(referent, ErrorMessage.format(ErrorMessageKeys.SHARED_STYLE_NOT_FOUND, styleUrl));
			ETSAssert.assertQualifiedName(referent,
					new QName[] { new QName(KML2.NS_NAME, "Style"), new QName(KML2.NS_NAME, "StyleMap") });
		}
	}

	/**
	 * [Test] Checks that the content of a kml:Region element satisfies all applicable
	 * constraints.
	 *
	 * @see RegionValidator
	 */
	...
	public void validRegion() {
		...
		for (int i = 0; i < targetElements.getLength(); i++) {
			Element kmlFeature = (Element) targetElements.item(i);
			NodeList region = kmlFeature.getElementsByTagNameNS(KML2.NS_NAME, "Region");
			if (region.getLength() == 0) {
				continue;
			}
			Assert.assertTrue(regionValidator.isValid(region.item(0)), regionValidator.getErrorMessages());
		}

	}

	/**
	 * [Test] Checks that the content of a kml:Camera or kml:LookAt element satisfies all
	...
	/**
	 * [Test] Checks that the content of a kml:ExtendedData element satisfies all
	 * applicable constraints.
	 *
	...
	public void validExtendedData() {
		...
		for (int i = 0; i < targetElements.getLength(); i++) {
			Element kmlFeature = (Element) targetElements.item(i);
			Node extData = kmlFeature.getElementsByTagNameNS(KML2.NS_NAME, "ExtendedData").item(0);
			if (null == extData) {
				continue;
			}
			Assert.assertTrue(validator.isValid(extData), validator.getErrorMessages());
		}
	}

	/**
	 * [Test] Checks that the value of the kml:phoneNumber element is a valid 'tel' URI
	 * that complies with RFC 3966, <em>The tel URI for Telephone Numbers</em>.
	 *
	 * @see <a href="http://tools.ietf.org/html/rfc3966" target="_blank">RFC 3966</a>
	 */
	...
	public void phoneNumber() {
		...
		for (int i = 0; i < targetElements.getLength(); i++) {
			Element kmlFeature = (Element) targetElements.item(i);
			Node phoneNum = kmlFeature.getElementsByTagNameNS(KML2.NS_NAME, "phoneNumber").item(0);
			if (null == phoneNum) {
				continue;
			}
			Matcher matcher = phoneNumPattern.matcher(phoneNum.getTextContent());
			Assert.assertTrue(matcher.matches(), ErrorMessage.format(ErrorMessageKeys.CONSTRAINT_VIOLATION,
					"Valid 'tel' URI (RFC 3966)", XMLUtils.buildXPointer(phoneNum)));
		}
	}
	 ...
	 *
	 * <p>
	 * The content model is specified by the <code>atomPersonConstruct</code> pattern in
	 * the RELAX NG schema. Note that within a container element authorship is inherited
	 * by all child feature members; it may be overridden on a per-feature basis.
	 * </p>
	 *
	 * @see <a href="http://tools.ietf.org/html/rfc4287#section-3.2" target="_blank">RFC
	 * 4287 - Person Constructs</a>
	 * @see <a href="http://tools.ietf.org/html/rfc5322#section-3.4.1" target="_blank">RFC
	 * 5322 - Addr-Spec Specification</a>
	 */
	...
	public void atomAuthor() {
		...
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			try {
				...
				Node email = XMLUtils.evaluateXPath(kmlFeature, "atom:author/atom:email", null).item(0);
				if (null != email) {
	...
	/**
	 * [Test] Checks various Schematron constraints that apply to any KML feature:
	 * <ul>
