...
/**
 * Checks constraints to apply to kml:ExtendedData elements. The relevant type definition
 * is shown below.
 *
 * <pre>
 * {@literal
...
public class ExtendedDataValidator {
	 ...
	 * @param doc The document containing the custom data.
	 */
	 ...
	 * @param node A kml:ExtendedData element.
	 ...
	 * <ol>
	 * <li>the value of the 'name' attribute is unique within the context of the parent
	 * kml:ExtendedData element</li>
	 * <li>if present, the uom attribute identifies a valid unit of measurement</li>
	 * </ol>
	 * @param extData A kml:ExtendedData element.
	 *
	...
	void checkData(Element extData) {
		NodeList dataList = extData.getElementsByTagNameNS(KML2.NS_NAME, "Data");
		...
		for (int i = 0; i < dataList.getLength(); i++) {
			...
			if (!nameSet.add(name)) {
				errHandler.addError(ErrorSeverity.ERROR, ErrorMessage.format(ErrorMessageKeys.DUPLICATE_DATA, name),
						new ErrorLocator(-1, -1, XMLUtils.buildXPointer(extData)));
			}
	...
	/**
	 * Checks that a kml:SchemaData element satisfies all applicable constraints:
	 * <ol>
	 * <li>the 'schemaUrl' attribute value is a URL (it may be an absolute URI) with a
	 * fragment component that refers to a kml:Schema element;</li>
	 * <li>all kml:SimpleData child elements have a 'name' attribute that matches the name
	 * of a declared kml:SimpleField element in the associated kml:Schema element (see
	 * ATC-126);</li>
	...
	void checkSchemaData(Element extData) {
		NodeList schemaDataList = extData.getElementsByTagNameNS(KML2.NS_NAME, "SchemaData");
		for (int i = 0; i < schemaDataList.getLength(); i++) {
			...
			if (schemaUrl.isEmpty()) {
				errHandler.addError(ErrorSeverity.ERROR,
						ErrorMessage.format(ErrorMessageKeys.MISSING_INFOSET_ITEM, "@schemaUrl"),
						new ErrorLocator(-1, -1, XMLUtils.buildXPointer(schemaData)));
				continue;
			}
			...
			try {
				...
				errHandler.addError(ErrorSeverity.ERROR, ErrorMessage.format(ErrorMessageKeys.NOT_FOUND, schemaURI),
						new ErrorLocator(-1, -1, XMLUtils.buildXPointer(schemaData)));
				continue;
			}
	...
	/**
	 * Validates the content of a kml:SchemaData element against the associated custom
	 * schema. In particular, for each data element:
	 * <ol>
	 * <li>it has a 'name' attribute that matches the name of a declared field in the
	 * associated schema;</li>
	 * <li>all values conform to the declared type.</li>
	 * </ol>
	 * @param schemaData A kml:SchemaData element containing kml:SimpleData or
	 * kml:SimpleArrayData elements.
	 * @param schemaFields A Map containing information (name, type) about the fields
		...
		for (int i = 0; i < dataNodes.getLength(); i++) {
			...
			for (String value : dataValues) {
				try {
					...
					XdmAtomicValue xdmValue = new XdmAtomicValue(value, schemaFields.get(name));
				}
				catch (SaxonApiException e) {
					errHandler.addError(ErrorSeverity.ERROR,
							ErrorMessage.format(ErrorMessageKeys.CONSTRAINT_VIOLATION, e.getMessage()),
							new ErrorLocator(-1, -1, XMLUtils.buildXPointer(data)));
				}
			}
		}
	 ...
	 * @param data A kml:SimpleData or kml:SimpleArrayData element.
	...
	String[] getDataValues(Element data) {
		...
		if (data.getLocalName().equals("SimpleData")) {
			...
			NodeList valueList = data.getElementsByTagNameNS(KML2.NS_NAME, "value");
	...
	/**
	 * Fetches the kml:Schema element referenced by the given URI. A relative URI is
	 * expected to be a same-document reference.
	 * @param schemaURI A URI containing a fragment identifier that refers to a kml:Schema
	 * element.
	 * @return An XdmNode representing a kml:Schema element.
	 * @throws SaxonApiException If an error occurs while trying to retrieve the target
	 * resource.
	 * @throws RuntimeException If a matching kml:Schema element cannot be obtained.
	 */
	XdmNode fetchSchema(URI schemaURI) throws SaxonApiException, RuntimeException {
		...
		Source src = new DOMSource(this.ownerDocument, this.ownerDocument.getBaseURI());
