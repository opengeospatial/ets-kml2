 ...
 * </ul>
 *
 * The content of a kml:coordinates element is a list of white space-separated 2D or 3D
 * tuples that contain comma-separated decimal values (lon,lat[,hgt]). The relevant schema
 * components are shown below. The relevant schema components are shown below.
 *
 * <pre>
 * {@literal
 ...
 *
 * <p>
 * The OGC KML specifications define a compound geographic 3D coordinate reference system
 * in Annex B.
 * </p>
 */
public class CoordinatesValidator {
	 ...
	 * </ol>
	 * @param node An Element node that contains a kml:coordinates element.
	 * @return true if the tuple sequence is valid; false otherwise.
	 */
	public boolean isValid(Node node) {
		errHandler.reset();
		Element elem = (Element) node;
		NodeList coords = elem.getElementsByTagNameNS(KML2.NS_NAME, "coordinates");
		if (coords.getLength() == 0) {
			errHandler.addError(ErrorSeverity.ERROR, ErrorMessage.get(ErrorMessageKeys.MISSING_COORDS),
			...
			case "Point":
				if (tuples.length != 1) {
					errHandler.addError(ErrorSeverity.ERROR,
							ErrorMessage.format(ErrorMessageKeys.POINT_COORDS, tuples.length),
							new ErrorLocator(-1, -1, XMLUtils.buildXPointer(node)));
				}
				break;
			case "LineString":
				if (tuples.length < 2) {
					errHandler.addError(ErrorSeverity.ERROR,
							ErrorMessage.format(ErrorMessageKeys.LINE_COORDS, tuples.length),
							new ErrorLocator(-1, -1, XMLUtils.buildXPointer(node)));
				}
				break;
			case "LinearRing":
				if (tuples.length < 4) {
					errHandler.addError(ErrorSeverity.ERROR,
							ErrorMessage.format(ErrorMessageKeys.RING_COORDS, tuples.length),
							new ErrorLocator(-1, -1, XMLUtils.buildXPointer(node)));
				}
				break;
			case "LatLonQuad":
				if (tuples.length != 4) {
					errHandler.addError(ErrorSeverity.ERROR,
							ErrorMessage.format(ErrorMessageKeys.QUAD_COORDS, tuples.length),
							new ErrorLocator(-1, -1, XMLUtils.buildXPointer(node)));
				}
				break;
			default:
		...
		for (int i = 0; i < tuples.length; i++) {
			...
			if (tuple.length < 2 || tuple.length > 3) {
				errHandler.addError(ErrorSeverity.ERROR,
						ErrorMessage.format(ErrorMessageKeys.COORD_DIM, i, "2-3", tuple.length),
						new ErrorLocator(-1, -1, XMLUtils.buildXPointer(node)));
				continue;
			}
			// ATC-202
			...
			if (altMode != AltitudeMode.CLAMP_TO_GROUND && tuple.length != 3) {
				errHandler.addError(
						ErrorSeverity.ERROR, ErrorMessage.format(ErrorMessageKeys.COORD_DIM, i,
			...
			for (String val : tuple) {
				try {
					...
					errHandler.addError(ErrorSeverity.ERROR, ErrorMessage.format(ErrorMessageKeys.NAN, val),
							new ErrorLocator(-1, -1, XMLUtils.buildXPointer(node)));
					continue;
				}
			}
