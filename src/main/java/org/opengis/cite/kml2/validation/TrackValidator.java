...
public class TrackValidator {
	...
	/**
	 * Returns the error messages reported during the last call to <code>isValid</code>.
	...
	/**
	 * Validates the content of a kml:Track element by checking the constraints defined by
	 ...
	 * </ol>
	 * @param node A kml:Track element.
	 * @return true if the track satisfies all applicable constraints; false otherwise.
	 */
	public boolean isValid(Node node) {
		...
		if (track.getElementsByTagNameNS(KML2.NS_NAME, "when")
			.getLength() != track.getElementsByTagNameNS(KML2.NS_NAME, "coord").getLength()) {
			errHandler.addError(ErrorSeverity.ERROR,
					...
					new ErrorLocator(-1, -1, XMLUtils.buildXPointer(track)));
		}
		checkAngles(track);
	...
	/**
	 * Verifies that the coordinates specifying the track positions (kml:coord) consist of
	 * space-delimited numeric values in the applicable coordinate reference system
	 * (default CRS).
	 * @param track A kml:Track element
	 *
	 * @see "OGC 14-068r1: OGC KML 2.3 - Abstract Test Suite, ATC-146"
	 */
	void checkCoordList(Element track) {
		NodeList coordList = track.getElementsByTagNameNS(KML2.NS_NAME, "coord");
		if (coordList.getLength() == 0) {
			errHandler.addError(ErrorSeverity.ERROR,
					ErrorMessage.format(ErrorMessageKeys.CONSTRAINT_VIOLATION, "Expected kml:coord in Track", ""),
					new ErrorLocator(-1, -1, XMLUtils.buildXPointer(track)));
			return;
		}
		for (int i = 0; i < coordList.getLength(); i++) {
			...
			if (coord.length != 3) {
				errHandler.addError(ErrorSeverity.ERROR,
						ErrorMessage.format(ErrorMessageKeys.COORD_DIM, i, "3", coord.length),
						new ErrorLocator(-1, -1, XMLUtils.buildXPointer(coordNode)));
				continue;
			}
			Coordinate jtsCoord = new Coordinate(coord[0], coord[1], coord[2]);
			if (!CRS_EXTENT.intersects(jtsCoord)) {
				errHandler.addError(ErrorSeverity.ERROR,
						ErrorMessage.format(ErrorMessageKeys.OUTSIDE_CRS, jtsCoord.toString()),
						new ErrorLocator(-1, -1, XMLUtils.buildXPointer(coordNode)));
			}
		}
	}
	...
	/**
	 * Verifies that each kml:angles element contained by a Track satisfies the following
	 * constraints: (a) it contains a space-delimited list of numeric values; (b) if the
	 * track has no associated model, then only one angle (heading) is specified.
	 * @param track A kml:Track element
	 *
	 * @see "OGC 14-068r1: OGC KML 2.3 - Abstract Test Suite, ATC-148"
	 */
	void checkAngles(Element track) {
		NodeList anglesList = track.getElementsByTagNameNS(KML2.NS_NAME, "angles");
		for (int i = 0; i < anglesList.getLength(); i++) {
			...
			if (null == track.getElementsByTagNameNS(KML2.NS_NAME, "Model").item(0) && (angles.length > 1)) {
				errHandler.addError(ErrorSeverity.ERROR,
						ErrorMessage.format(ErrorMessageKeys.CONSTRAINT_VIOLATION,
								"[ATC-148] Expected one angle value, since Track has no Model", ""),
						new ErrorLocator(-1, -1, XMLUtils.buildXPointer(anglesNode)));
				continue;
			}
		}
	}
	 ...
	 * @param node A node containing a sequence of space-separated values, all of which
	 * are presumed to be a parsable double.
	...
	double[] getNumericValues(Node node) {
		...
		for (int j = 0; j < values.length; j++) {
			try {
				...
				errHandler.addError(ErrorSeverity.ERROR, ErrorMessage.format(ErrorMessageKeys.NAN, values[j]),
