...
/**
 * The relevant type definition is shown below (with extension points omitted); the
 * spatial extent of the region must be specified using a kml:LatLonAltBox element.
 *
 * <pre>
 * {@literal
...
public class RegionValidator {
	...
	/**
	 * Constructs a RegionValidator to check the constraints that apply to the specified
	 * conformance level.
	 ...
	 * @param node A kml:Region element.
	 * @return true if the region satisfies all applicable constraints; false otherwise.
	 */
	...
	/**
	 * Checks the spatial extent of a Region (kml:LatLonAltBox). The default extent of a
	 * region spans the entire surface of the EGM96 geoid.
	 * @param region A kml:Region element.
	 */
	void checkRegionExtent(Element region) {
		Node boxNode = region.getElementsByTagNameNS(KML2.NS_NAME, "LatLonAltBox").item(0);
		if (null == boxNode) {
			errHandler.addError(ErrorSeverity.ERROR,
					ErrorMessage.format(ErrorMessageKeys.CONSTRAINT_VIOLATION, "[ATC-140] Expected LatLonAtlBox"),
	...
	void checkRegionVisibility(Element region) {
		Node lodNode = region.getElementsByTagNameNS(KML2.NS_NAME, "Lod").item(0);
		if (null == lodNode) {
			errHandler.addError(ErrorSeverity.ERROR,
					ErrorMessage.format(ErrorMessageKeys.CONSTRAINT_VIOLATION, "[ATC-140] Expected Lod"),
					new ErrorLocator(-1, -1, XMLUtils.buildXPointer(region)));
			return;
		}
		Node minLodPixelsNode = region.getElementsByTagNameNS(KML2.NS_NAME, "minLodPixels").item(0);
		...
		Node maxLodPixelsNode = region.getElementsByTagNameNS(KML2.NS_NAME, "maxLodPixels").item(0);
	...
	/**
	 * Checks that the following expression holds for the pixel ramp values that determine
	 * whether or not a region is active:
	 *
	 * <pre>
	 * {@literal
	 ...
	 * @param region A kml:Region element.
	 * @param paramName The (local) name of some Lod parameter.
	 * @return The actual value of the parameter (or its specified default value).
	 */
	double getLodParameterValue(Element region, String paramName) {
		...
		Node lodParam = region.getElementsByTagNameNS(KML2.NS_NAME, paramName).item(0);
		if (null != lodParam) {
