...
/**
 * The relevant type definition for kml:Update is shown below; it may appear within a
 * NetworkLinkControl or AnimatedUpdate element. The elements that may substitute for
 * <em>kml:AbstractUpdateOptionGroup</em> are kml:Create, kml:Change and kml:Delete.
 *
 * <pre>
 * {@literal
 ...
 *     <xsd:element ref="kml:AbstractUpdateOptionGroup" minOccurs="1" maxOccurs=
 *"unbounded"/>
...
public class UpdateValidator {
	 ...
	 * particular:
	 * <ol>
	 * <li>if it is not empty, the kml:targetHref element contains an absolute URI that
	 * refers to a KML or KMZ resource.</li>
	 ...
	 * </ol>
	 *
	 * In the context of an AnimatedUpdate appearing in a Tour, an empty Update/targetHref
	 * element indicates that all elements to be updated occur in the same document.
	 * @param updateNode A node that represents a kml:Update element.
	...
	public boolean isValid(Node updateNode) {
		...
		Element targetUri = (Element) update.getElementsByTagNameNS(KML2.NS_NAME, "targetHref").item(0);
		try {
			if (!targetUri.getTextContent().trim().isEmpty()) {
				ETSAssert.assertReferentExists(URI.create(targetUri.getTextContent().trim()),
						MediaType.valueOf(KML2.KML_MEDIA_TYPE), MediaType.valueOf(KML2.KMZ_MEDIA_TYPE));
			}
