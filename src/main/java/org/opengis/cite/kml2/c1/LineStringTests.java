...
/**
 * Implements tests that apply to kml:LineString elements. The relevant test cases from
...
public class LineStringTests extends CommonFixture {
	...
	/**
	 * <p>
	 * Constructor for LineStringTests.
	 * </p>
	 */
	...
	/**
	 * Finds kml:LineString elements in the KML document that do not appear in an update
	...
	/**
	 * [Test] Verifies that a kml:LineString element has valid coordinates. It must
	 * contain two or more coordinate tuple in the default CRS.
	 */
	...
	public void validLineStringCoordinates() {
		...
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			Assert.assertTrue(coordsValidator.isValid(line), coordsValidator.getErrorMessages());
			LineString jtsLine = geomBuilder.buildLineString(line);
			Assert.assertTrue(crsPolygon.covers(jtsLine),
					ErrorMessage.format(ErrorMessageKeys.OUTSIDE_CRS, jtsLine.toText()));
		}
	}

	/**
	 * [Test] Verifies that a kml:LineString element has a valid altitudeMode value as
