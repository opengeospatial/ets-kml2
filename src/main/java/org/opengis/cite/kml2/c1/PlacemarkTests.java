...
/**
 * Implements tests that apply to the kml:Placemark element. A Placemark is a feature that
 * may have an associated geometry element (that substitutes for
...
public class PlacemarkTests extends CommonFeatureTests {
	...
	/**
	 * <p>
	 * Constructor for PlacemarkTests.
	 * </p>
	 */
	...
	/**
	 * Finds kml:Placemark elements in the KML resource that do not appear in an update
	...
	public void validTrack() {
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			Node track = place.getElementsByTagNameNS(KML2.NS_NAME, "Track").item(0);
			if (null == track) {
				continue;
			}
			Assert.assertTrue(trackValidator.isValid(track), trackValidator.getErrorMessages());
		}
	}
