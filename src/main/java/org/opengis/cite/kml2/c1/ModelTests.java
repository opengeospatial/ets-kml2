...
/**
 * Implements tests that apply to kml:Model elements. Outside of an update context the
 * following constraints apply:
 * <ul>
 * <li>contains a kml:Link element (specifies the location of a textured 3D object
 * resource)</li>
 * <li>contains a kml:Location element (specifies the position of the model's origin)</li>
 * <li>if the target resource has texture files, contains a kml:ResourceMap element that
 * has a kml:Alias element for each texture file</li>
 * </ul>
 *
...
public class ModelTests extends CommonFixture {
	...
	/**
	 * <p>
	 * Constructor for ModelTests.
	 * </p>
	 */
	public ModelTests() {
		this.modelLinkValidator = new LinkValidator(MediaType.valueOf("model/*"));
	}

	/**
	 * Finds kml:Model elements in the KML document that do not appear in an update
	...
	/**
	 * [Test] Verifies that a kml:Model element has a valid location (that specifies the
	 * position of the model origin).
	 */
	...
	public void modelLocation() {
		...
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			NodeList location = model.getElementsByTagNameNS(KML2.NS_NAME, "Location");
			if (location.getLength() == 0) {
				throw new AssertionError(ErrorMessage.format(ErrorMessageKeys.MISSING_INFOSET_ITEM, "kml:Location",
	...
	/**
	 * [Test] Verifies that a kml:Model element has a valid reference to a textured 3D
	 * object resource. The referent must exist, but the content is not prescribed by the
	 * KML specification. In practice this is usually a COLLADA file
	...
	public void modelLink() {
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			Assert.assertTrue(modelLinkValidator.isValid(link.item(0)), modelLinkValidator.getErrorMessages());
		}
	}

	/**
	 * [Test] Verifies that a kml:Orientation element contains at least one of the
	...
	public void orientation() {
		for (int i = 0; i < targetElements.getLength(); i++) {
			Element model = (Element) targetElements.item(i);
			Element orientation = (Element) model.getElementsByTagNameNS(KML2.NS_NAME, "Orientation").item(0);
			if (null == orientation) {
				continue;
			}
			Assert.assertTrue(orientation.getElementsByTagNameNS(KML2.NS_NAME, "*").getLength() > 0,
					ErrorMessage.format(ErrorMessageKeys.CONSTRAINT_VIOLATION, "Model orientation is empty.",
							XMLUtils.buildXPointer(orientation)));
		}
	}

	/**
	 * [Test] Verifies that the kml:Alias elements appearing in a kml:ResourceMap element
	 * satisfy all of the following constraints:
	 * <ol>
	 * <li>the value of the child kml:targetHref element is a URI reference to an image
	 * (texture) resource;</li>
	 * <li>the value of the child kml:sourceHref element corresponds to a file reference
	 * that appears within the 3D object resource referenced by the sibling kml:Link
	 * element</li>
	 * </ol>
	 *
	 * Note: The source file is expected to be a textual (including XML) digital asset
	 * resource such as a COLLADA file.
	 */
	@Test(description = "ATC-129, ATC-209")
	public void resourceMap() {
		for (int i = 0; i < targetElements.getLength(); i++) {
			Element model = (Element) targetElements.item(i);
			Element map = (Element) model.getElementsByTagNameNS(KML2.NS_NAME, "ResourceMap").item(0);
			if (null == map) {
				continue;
			}
			NodeList aliases = map.getElementsByTagNameNS(KML2.NS_NAME, "Alias");
			if (this.conformanceLevel > 1) {
				Assert.assertTrue(aliases.getLength() > 0, ErrorMessage.format(ErrorMessageKeys.CONSTRAINT_VIOLATION,
	 ...
	 * @param alias A kml:Alias element.
	 * @param modelURI An absolute URI that refers to a 3D model (e.g. a COLLADA file);
	 * this will be used to resolve relative source URIs if necessary.
	 */
	void assertValidAlias(Element alias, URI modelURI) {
		Element sourceHrefElem = (Element) alias.getElementsByTagNameNS(KML2.NS_NAME, "sourceHref").item(0);
		Assert.assertNotNull(sourceHrefElem, ErrorMessage.format(ErrorMessageKeys.MISSING_INFOSET_ITEM,
				"kml:sourceHref", XMLUtils.buildXPointer(alias)));
		...
		Element targetHref = (Element) alias.getElementsByTagNameNS(KML2.NS_NAME, "targetHref").item(0);
		Assert.assertNotNull(targetHref, ErrorMessage.format(ErrorMessageKeys.MISSING_INFOSET_ITEM, "kml:targetHref",
