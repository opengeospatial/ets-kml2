...
/**
 * Checks constraints that apply to a KML resource as a whole. Specifically, it must be
 * schema-valid. The applicable schema is determined by the value of the version attribute
 * set on the document element (default value: "2.2").
 *
...
public class KmlResourceTests extends CommonFixture {
	...
	/**
	 * [Test] Verifies that the KML document is valid with respect to the applicable XML
	 * Schema grammar (as determined by the value of the version attribute).
	 */
	@Test(description = "ATC-102")
	public void isSchemaValid() {
		Schema kmlSchema;
		String kmlVer = this.kmlDoc.getDocumentElement().getAttribute(KML2.VER_ATTR);
