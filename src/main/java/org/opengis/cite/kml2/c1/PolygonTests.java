...
/**
 * Implements tests that apply to kml:Polygon elements. The relevant test cases from the
...
public class PolygonTests extends CommonFixture {
	...
	/**
	 * Finds kml:Polygon elements in the KML document that do not appear in an update
	 ...
	 * topologically correct. All boundary elements must be a valid LinearRing (see
	 * ATC-116). Each interior boundary, if present, must define a "hole" in the Polygon
	 * such that it lies within the exterior boundary.
	 */
	...
	public void validPolygonBoundary() {
		...
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			Assert.assertTrue(outerRing.getLength() == 1, ErrorMessage.format(ErrorMessageKeys.POLYGON_BOUNDARY,
					"Missing outer boundary", XMLUtils.buildXPointer(polygon)));
			Assert.assertTrue(coordsValidator.isValid(outerRing.item(0)), coordsValidator.getErrorMessages());
			Polygon jtsPolygon = null;
			try {
				jtsPolygon = geomBuilder.buildPolygon(polygon);
			}
			catch (IllegalArgumentException ex) {
				throw new AssertionError(ErrorMessage.format(ErrorMessageKeys.POLYGON_BOUNDARY, ex.getMessage(),
						XMLUtils.buildXPointer(polygon)));
			}
			Assert.assertTrue(crsPolygon.covers(jtsPolygon),
					ErrorMessage.format(ErrorMessageKeys.OUTSIDE_CRS, jtsPolygon.toText()));
		}
	}

	/**
	 * [Test] Verifies that a kml:Polygon element has a valid altitudeMode value as
	 * determined by the values of its kml:extrude and kml:tessellate elements.
	 */
