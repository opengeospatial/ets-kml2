...
public class TourTests extends CommonFeatureTests {
	...
	/**
	 * Finds kml:Tour elements in the KML resource that do not appear in an update
	...
	/**
	 * [Test] Verifies that the kml:SoundCue elements in a tour refer to an audio resource
	 * (of media type <code>audio/*</code>). Some commonly used audio formats are listed
	 * below.
	 *
	 * <ul>
	...
	public void soundCue() {
		for (int i = 0; i < targetElements.getLength(); i++) {
			Element tour = (Element) targetElements.item(i);
			NodeList soundCues = tour.getElementsByTagNameNS(KML2.NS_NAME, "SoundCue");
			for (int j = 0; j < soundCues.getLength(); j++) {
				Element soundCue = (Element) soundCues.item(j);
				Node href = soundCue.getElementsByTagNameNS(KML2.NS_NAME, "href").item(0);
				Assert.assertNotNull(href, ErrorMessage.format(ErrorMessageKeys.MISSING_INFOSET_ITEM, "kml:href",
				...
				ETSAssert.assertReferentExists(uri, MediaType.valueOf("audio/*"));
			}
		}
	}

	/**
	 * [Test] Verifies that a kml:AnimatedUpdate element satisfies all applicable
	 * constraints. Specifically, the update targets must exist (possibly in the same
	 * document).
	 */
	...
	public void animatedUpdateTarget() {
		for (int i = 0; i < targetElements.getLength(); i++) {
			Element tour = (Element) targetElements.item(i);
			NodeList updates = tour.getElementsByTagNameNS(KML2.NS_NAME, "Update");
			for (int j = 0; j < updates.getLength(); j++) {
				Assert.assertTrue(updateValidator.isValid(updates.item(j)), updateValidator.getErrorMessages());
			}
		}
	}
