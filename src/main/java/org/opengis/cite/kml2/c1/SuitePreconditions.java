...
public class SuitePreconditions {
	...
	/**
	 * Verifies that the document (root) element has the following infoset properties:
	 * <ul>
	...
	public void isKMLDocument() {
		...
		if (null != sut && Document.class.isInstance(sut)) {
			...
			ETSAssert.assertQualifiedName(doc.getDocumentElement(), new QName(KML2.NS_NAME, KML2.DOC_ELEMENT));
		}
		else {
			String msg = String.format(
					"Value of test suite attribute %s is missing or does not refer to an XML resource.",
	...
	public void checkVersion() {
		...
		if (null != ver) {
			...
			Assert.assertTrue(version.startsWith("2.2") || version.startsWith("2.3"),
					ErrorMessage.format(ErrorMessageKeys.UNSUPPORTED_VERSION, version));
		}
		else {
			String msg = String.format("Value of test suite attribute %s is missing.",
