...
/**
 * Implements tests that apply to container features (of type kml:AbstractContainerType):
 * kml:Document and kml:Folder.
 *
...
public class ContainerTests extends CommonFeatureTests {
	...
	/**
	 * <p>
	 * Constructor for ContainerTests.
	 * </p>
	 */
	...
	/**
	 * Finds shared styles occurring in a kml:Document container. The resulting collection
	 * ({@code Set<String>}) of style identifiers is set as the value of the suite
	 * attribute {@link org.opengis.cite.kml2.SuiteAttribute#SHARED_STYLES}.
	 */
	@BeforeSuite
	public void findSharedStyles() {
	...
	/**
	 * Finds custom schemas (kml:Schema) defined in a kml:Document container. The
	 * resulting collection ({@code Set<String>}) of schema identifiers is set as the
	 * value of the suite attribute
	 * {@link org.opengis.cite.kml2.SuiteAttribute#CUSTOM_SCHEMAS}.
	 */
	@BeforeSuite
	public void findCustomSchemas() {
		ITestContext testContext = Reporter.getCurrentTestResult().getTestContext();
		Document kmlDoc = (Document) testContext.getSuite().getAttribute(SuiteAttribute.TEST_SUBJECT.getName());
		Source kmlSource = new DOMSource(kmlDoc, kmlDoc.getBaseURI());
	...
	/**
	 * Finds container elements (Folder, Document) in the KML document that do not appear
	 * in an update context. If none are found, all test methods defined in the class are
	 * skipped.
	 */
	@BeforeClass
	...
	/**
	 * [Test] Verifies that a kml:Schema element (appearing in a kml:Document context) is
	 * defined properly.
	 */
	...
	public void validSchema() {
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			NodeList schemaList = container.getElementsByTagNameNS(KML2.NS_NAME, "Schema");
			for (int j = 0; j < schemaList.getLength(); j++) {
				Assert.assertTrue(schemaChecker.isValid(schemaList.item(j)), schemaChecker.getErrorMessages());
			}
		}
	}
