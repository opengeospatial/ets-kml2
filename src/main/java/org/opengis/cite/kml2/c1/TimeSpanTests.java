...
/**
 * Implements tests that apply to kml:TimeSpan elements. The relevant test cases from the
...
public class TimeSpanTests extends CommonFixture {
	...
	/**
	 * Finds kml:TimeSpan elements in the KML document that do not appear in an update
	...
	/**
	 * [Test] Verifies that a kml:TimeSpan element includes at least one child element
	 * (kml:begin or kml:end). If it is a definite interval (both kml:begin and kml:end
	 * are present), then the end value must be later than the begin value.
	 *
	 * <p>
	 * Temporal values of varying precision (from fractional second to year) are
	 * permitted: xsd:dateTime, xsd:date, xsd:gYearMonth, and xsd:gYear. A time zone
	...
	public void validTimeSpan() {
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			try {
				...
				Assert.assertFalse(nodes.getLength() == 0,
						ErrorMessage.format(ErrorMessageKeys.EMPTY_TIMESPAN, XMLUtils.buildXPointer(span)));
			}
			...
			if ((nodes.getLength() == 2) && !isValidDefiniteTimeInterval(nodes)) {
				ValidationError err = new ValidationError(ErrorSeverity.ERROR,
						ErrorMessage.get(ErrorMessageKeys.TIMESPAN_INTERVAL), null, -1, -1,
	 ...
	 * @param timeInstants A pair of nodes containing the beginning and ending instants of
	 * the interval (kml:begin, kml:end).
	...
	boolean isValidDefiniteTimeInterval(NodeList timeInstants) {
		...
		for (int i = 0; i < timeInstants.getLength(); i++) {
			...
			try {
				DateTime dateTime = timeFormatter.parseDateTime(instant.getTextContent().trim());
