...
public class CL2LinearRingTests extends CommonFixture {
	...
	/**
	 * [Test] Verifies that a kml:LinearRing element that constitutes the boundary of a
	...
	public void linearRingInPolygonBoundary() {
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			if (ring.getParentNode().getLocalName().endsWith("BoundaryIs")) {
				ETSAssert.assertXPath("not(kml:extrude or kml:tesselate or kml:altitudeMode)", ring, null);
			}
		}
	}
