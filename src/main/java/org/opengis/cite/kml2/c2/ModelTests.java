...
public class ModelTests extends CommonFixture {
	 ...
	 * That is, at least one scaling factor has a non-default value.
	 */
	@Test(description = "ATC-203")
	public void scaleNotEmpty() {
		for (int i = 0; i < targetElements.getLength(); i++) {
			Element model = (Element) targetElements.item(i);
			NodeList items = model.getElementsByTagNameNS(KML2.NS_NAME, "Scale");
			if (items.getLength() > 0 && !items.item(0).hasChildNodes()) {
				throw new AssertionError(
						ErrorMessage.format(ErrorMessageKeys.NO_CONTENT, "kml:Scale", XMLUtils.buildXPointer(model)));
			}
		}
	}
