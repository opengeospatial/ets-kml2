...
public class CL2OverlayTests extends CommonFixture {
	...
	/**
	 * Finds elements that substitute for kml:AbstractOverlayGroup elements and do not
	...
	/**
	 * [Test] Verifies that if a kml:PhotoOverlay element includes a kml:Icon/kml:href
	 * element containing one or more tiling parameters (e.g. level, x, y), then it also
	 * includes a child kml:ImagePyramid element; the converse must also be true (i.e.
	...
	public void photoOverlayForTiledImage() {
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			try {
				...
				throw new AssertionError(ErrorMessage.format(ErrorMessageKeys.CONSTRAINT_VIOLATION,
						"Tiling parameters and ImagePyramid are mutually inclusive", XMLUtils.buildXPointer(overlay)));
			}
		}
	}

	/**
	 * [Test] Verify that the geographic extent of a GroundOverlay feature is specified by
	 * either a kml:LatLonBox or a kml:LatLonQuad element.
	 */
	...
	public void groundOverlayExtent() {
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			try {
				...
				throw new AssertionError(ErrorMessage.format(ErrorMessageKeys.CONSTRAINT_VIOLATION,
						"Expected kml:LatLonBox or kml:LatLonQuad", XMLUtils.buildXPointer(overlay)));
			}
		}
	}

	/**
	 * [Test] Verify that a ScreenOverlay has a child kml:screenXY element. It specifies a
	...
	public void screenOverlayPosition() {
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			try {
				...
				// provide more informative error message
				throw new AssertionError(ErrorMessage.format(ErrorMessageKeys.CONSTRAINT_VIOLATION,
						"Expected kml:screenXY in ScreenOverlay", XMLUtils.buildXPointer(overlay)));
			}
		}
	}

	/**
	 * [Test] Verify that an overlay feature contains a kml:Icon child element that refers
	...
	public void overlayImage() {
		...
		for (int i = 0; i < targetElements.getLength(); i++) {
			...
			Assert.assertNotNull(icon, ErrorMessage.format(ErrorMessageKeys.MISSING_INFOSET_ITEM, "kml:Icon",
