...
public class JTSGeometryBuilder {
	...
	/**
	 * <p>
	 * Constructor for JTSGeometryBuilder.
	 * </p>
	 */
	 ...
	 * @param point An Element node (kml:Point).
	 * @return A JTS Point.
	 */
	public Point buildPoint(Element point) {
		...
		String[] tuple = point.getElementsByTagNameNS(KML2.NS_NAME, "coordinates")
			.item(0)
			.getTextContent()
			.trim()
			.split(",");
	 ...
	 * @param location An Element node (kml:Location).
	 * @return A JTS Point.
	 */
	public Point buildPointFromLocation(Element location) {
		...
		NodeList nodes = location.getElementsByTagNameNS(KML2.NS_NAME, "longitude");
		double lon = (nodes.getLength() > 0) ? Double.parseDouble(nodes.item(0).getTextContent()) : 0;
		nodes = location.getElementsByTagNameNS(KML2.NS_NAME, "latitude");
		double lat = (nodes.getLength() > 0) ? Double.parseDouble(nodes.item(0).getTextContent()) : 0;
		nodes = location.getElementsByTagNameNS(KML2.NS_NAME, "altitude");
		double alt = (nodes.getLength() > 0) ? Double.parseDouble(nodes.item(0).getTextContent()) : 0;
	 ...
	 * @param line An Element node (kml:LineString).
	...
	public LineString buildLineString(Element line) {
		...
		List<Coordinate> coordList = createCoordinateList(
				line.getElementsByTagNameNS(KML2.NS_NAME, "coordinates").item(0));
	 ...
	 * @param ring An Element node (kml:LinearRing).
	...
	public LinearRing buildLinearRing(Element ring) {
		...
		List<Coordinate> coordList = createCoordinateList(
				ring.getElementsByTagNameNS(KML2.NS_NAME, "coordinates").item(0));
	 ...
	 * @param polygon An Element node (kml:Polygon).
	 * @return A JTS Polygon.
	 * @throws java.lang.IllegalArgumentException If a boundary element (ring) is not
	 * closed or an inner boundary is not inside the outer boundary.
	 */
		...
		for (int i = 0; i < rings.getLength(); i++) {
			...
			if (!outerPolygon.contains(innerRing)) {
				throw new IllegalArgumentException(
						String.format("Inner boundary [%d] not inside outer boundary.", i + 1));
			}
	 ...
	 * @param env A JTS envelope defining some spatial extent.
	 * @return A rectangular polygon that covers the same extent as the envelope.
	 */
	...
	/**
	 * Builds a Polygon with an exterior boundary delimited by the given sequence of
	 * coordinates. The sequence will be closed if necessary such that the first and last
	 * vertices are coincident.
	 * @param coords A node containing a list of coordinates (kml:coordinates).
	 ...
	 * @param coords A node containing a coordinate sequence (kml:coordinates).
	...
	List<Coordinate> createCoordinateList(Node coords) {
		...
		for (String tuple : tuples) {
			...
			double alt = (coordTuple.length > 2) ? Double.parseDouble(coordTuple[2]) : 0;
			Coordinate coord = new Coordinate(Double.parseDouble(coordTuple[0]), Double.parseDouble(coordTuple[1]),
					alt);
			coordList.add(coord);
		}
		return coordList;
