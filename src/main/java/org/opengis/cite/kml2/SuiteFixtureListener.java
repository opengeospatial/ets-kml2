...
public class SuiteFixtureListener implements ISuiteListener {
	 ...
	 * of a KML 2.x document. The schemas are added to the suite fixture as the value of
	 * the attributes identified in the following table.
	 *
	...
	void buildKMLSchemas(ISuite suite) {
		...
		if (null != kml22Schema) {
			suite.setAttribute(SuiteAttribute.KML22_SCHEMA.getName(), kml22Schema);
		}
		Schema kml23Schema = ValidationUtils.createKMLSchema("2.3");
		if (null != kml23Schema) {
			suite.setAttribute(SuiteAttribute.KML23_SCHEMA.getName(), kml23Schema);
		}
	}
	 ...
	 * referenced by the {@link TestRunArg#KML kml} argument is parsed (or unpacked first
	 * if it's a KMZ resource) and the resulting Document is set as the value of the
	 * "testSubject" attribute. The level of conformance assessment is determined by the
	 * value of the {@link TestRunArg#LVL lvl} argument (default value: 1).
	...
	void processSuiteParameters(ISuite suite) {
		...
		if (null != params.get(TestRunArg.LVL.toString())) {
			try {
				int lvlParam = Integer.parseInt(params.get(TestRunArg.LVL.toString()));
		...
		String kmlVersion = kmlDoc.getDocumentElement().getAttribute(KML2.VER_ATTR);
	...
	/**
	 * An JAX-RS Client component is added to the suite fixture as the value of the
